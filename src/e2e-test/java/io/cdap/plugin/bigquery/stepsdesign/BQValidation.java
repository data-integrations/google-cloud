...
public class BQValidation {
  private static List<JsonObject> getBigQueryTableData(String table)
    ...
    String sortingIdKey = getIdKey(bigQueryRows.get(0));
    // Sort the list based on the determined sorting key
    if (sortingIdKey != null) {
      bigQueryRows.sort(Comparator.comparing(jsonData -> jsonData.get(sortingIdKey).getAsString()));
    }
    return bigQueryRows;
  }
  ...
  private static String getIdKey(JsonObject json) {
    if (json.has("UID")) {
      return "UID";
    } else if (json.has("transaction_uid")) {
  ...
  public static boolean validateSourceBQToTargetBQRecord(String sourceTable, String targetTable)
    ...
    return compareSourceDataWithTargetData
      (bigQuerySourceResponse, bigQueryTargetResponse, targetTable);
  }

  public static boolean compareSourceDataWithTargetData(List<JsonObject> sourceData, List<JsonObject> targetData,
                                                        String tableName) {
    if (targetData == null) {
    ...
    for (int rowIndex = 0; rowIndex < sourceData.size(); rowIndex++) {
      ...
      for (Field field : schema.getFields()) {
        ...
        switch (columnTypeName) {
          ...
          case "INT64":
            int intSource = sourceObject.get(columnName).getAsInt();
            int intTarget = targetObject.get(columnName).getAsInt();
            Assert.assertEquals("Different values found for column : %s", intSource, intTarget);
            break;
