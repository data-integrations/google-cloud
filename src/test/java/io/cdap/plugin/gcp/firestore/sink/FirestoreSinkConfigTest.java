...
import static io.cdap.plugin.gcp.firestore.sink.FirestoreSinkConfigHelper.TEST_COLLECTION;
import static io.cdap.plugin.gcp.firestore.sink.FirestoreSinkConfigHelper.TEST_ID_ALIAS;
...
public class FirestoreSinkConfigTest {
  ...
  public void testGetIdTypeUnknown() {
    ...
    try {
      ...
      Assert.assertEquals(PROPERTY_ID_TYPE, e.getFailures().get(0).getCauses().get(0)
  ...
  public void testGetIdAliasNull() {
    ...
    Assert.assertEquals(ID_PROPERTY_NAME, config.getIdAlias());
  }

  @Test
  public void testGetIdAliasNotEmpty() {
    FirestoreSinkConfig config = FirestoreSinkConfigHelper.newConfigBuilder()
      .setIdAlias(TEST_ID_ALIAS)
      .build();

    Assert.assertEquals(TEST_ID_ALIAS, config.getIdAlias());
  }
  ...
  public void testIsUseAutoGeneratedIdFalse() {
    ...
    MockFailureCollector collector = new MockFailureCollector();
    Assert.assertFalse(config.shouldUseAutoGeneratedId(collector));
    Assert.assertEquals(0, collector.getValidationFailures().size());
  }
  ...
  public void testIsUseAutoGeneratedIdTrue() {
    ...
    MockFailureCollector collector = new MockFailureCollector();
    Assert.assertTrue(config.shouldUseAutoGeneratedId(collector));
    Assert.assertEquals(0, collector.getValidationFailures().size());
  }
  ...
  public void testValidateCustomIdWithKeyAliasInSchema() {
    ...
    Schema schema = Schema.recordOf("record",
      Schema.Field.of(TEST_ID_ALIAS, Schema.of(Schema.Type.STRING)),
  ...
  public void testValidateBatchSizeZero() {
    ...
    config.validate(schema, collector);
    Assert.assertEquals(1, collector.getValidationFailures().size());
    Assert.assertEquals(PROPERTY_BATCH_SIZE, collector.getValidationFailures().get(0)
      .getCauses().get(0).getAttribute(CauseAttributes.STAGE_CONFIG));
  }
  ...
  public void testValidateBatchOverMax() {
    int batchSize = MAX_BATCH_SIZE + 1;
    FirestoreSinkConfig config = Mockito.spy(FirestoreSinkConfigHelper.newConfigBuilder()
      .setIdType(SinkIdType.AUTO_GENERATED_ID.getValue())
      .setServiceFilePath(null)
      .setBatchSize(batchSize)
