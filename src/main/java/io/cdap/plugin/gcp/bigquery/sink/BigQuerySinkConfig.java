...
public final class BigQuerySinkConfig extends AbstractBigQuerySinkConfig {
  ...
  private void validateClusteringOrder(@Nullable Schema schema, FailureCollector collector) {
    ...
    if(!Strings.isNullOrEmpty(clusteringOrder) && Strings.isNullOrEmpty(partitionByField)){
      collector.addFailure(String.format("Clustering order cannot be validated."),"Partition field must have a value.");
      return;
    }
