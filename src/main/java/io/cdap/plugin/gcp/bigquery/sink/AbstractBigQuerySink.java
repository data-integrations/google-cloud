    ...
    if (allowSchemaRelaxation) {
      ...
      for (String missingField : missingBQFields) {
        collector.addFailure(String.format(
          "The BigQuery table '%s.%s' does not contain '%s' column.",
          getConfig().getDataset(), tableName, missingField),
                             "Schema must contain fields same as BigQuery table schema.")
          .withOutputSchemaField(missingField, null);
      }
      ...
      for (String field : remainingBQFields) {
        if (bqFields.get(field).getMode() != Field.Mode.NULLABLE) {
          collector.addFailure(String.format("The BigQuery table '%s.%s' requires column '%s' that is not " +
                                               "in the output schema.", getConfig().getDataset(), tableName, field),
                               "Schema must contain fields same as BigQuery table schema.");
        }
      }
    }
    ...
    for (Schema.Field field : tableSchema.getFields()) {
      ...
      if (!missingBQFields.contains(fieldName)) {
        BigQueryUtil.validateFieldSchemaMatches(bqFields.get(field.getName()),
                                                field, getConfig().getDataset(), tableName, collector);
      }
    }
