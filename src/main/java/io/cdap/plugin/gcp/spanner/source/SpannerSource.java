...
public class SpannerSource extends BatchSource<NullWritable, ResultSet, StructuredRecord> {
  ...
  public void configurePipeline(PipelineConfigurer pipelineConfigurer) {
    ...
    if (config.containsMacro("schema") || config.containsMacro("database") || config.containsMacro("table") ||
      config.containsMacro("instance") || config.containsMacro("serviceFilePath") ||
      config.containsMacro("project")) {
  ...
  private Schema parseSchemaFromSpannerTypeString(String columnName,
    ...
    if (spannerType.startsWith("ARRAY")) {
      ...
      switch (spannerType) {
        ...
        default:
          collector.addFailure(String.format("Column '%s' is an array, which is not currently supported.",
                                             columnName), null);
      }
