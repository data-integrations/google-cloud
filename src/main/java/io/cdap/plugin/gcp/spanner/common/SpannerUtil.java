    ...
    for (Schema.Field field : schema.getFields()) {
      ...
      if (logicalType != null && !SUPPORTED_LOGICAL_TYPES.contains(logicalType)) {
        collector.addFailure(
          String.format("Field schema of type '%s' is unsupported.", fieldSchema.getDisplayName()),
          "Supported types are: date and timestamp").withOutputSchemaField(field.getName(), null);
      }
