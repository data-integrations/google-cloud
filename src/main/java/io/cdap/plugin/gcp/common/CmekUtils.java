...
public class CmekUtils {
   ...
   */
  public static void validateCmekKeyAndBucketLocation(Storage storage, @Nullable GCSPath gcsPath, CryptoKeyName cmekKey,
    ...
    try {
      bucket = gcsPath == null ? null : storage.get(gcsPath.getBucket());
    ...
    if (bucket == null) {
      ...
      if (!cmekKeyLocation.equalsIgnoreCase(location)) {
        collector.addFailure(
          String.format(errorMessageTemplate, cmekKey.getCryptoKey(), cmekKeyLocation, "bucket",
                        gcsPath == null ? "temporary GCS bucket" : gcsPath.getBucket(), location),
  ...
  public static Dataset validateCmekKeyAndDatasetOrTableLocation(BigQuery bigQuery, DatasetId datasetId,
    ...
    if (dataset == null) {
      ...
      // dataset exists but table does not, so table will be created in the location of dataset.
  ...
  public static CryptoKeyName getCmekKey(@Nullable String configKey, Map<String, String> arguments,
                                         FailureCollector collector) {
    String cmekKey = configKey;
    if (Strings.isNullOrEmpty(cmekKey)) {
      cmekKey = arguments.get(CMEK_KEY);
    }
    return getCmekKey(cmekKey, collector);
  }
}
